    ...
    <div class="space-y-6">
            ...
            <form method="GET" action="{{ route('student.events.index') }}" class="flex items-center gap-3">
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by title..."
                        class="pl-9 pr-4 py-2 bg-white border border-gray-200/80 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder:text-gray-400 shadow-xs">
                ...
                <button type="submit"
                    class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs rounded-xl transition-all shadow-xs">
                    Filter
                </button>
            </form>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($events as $event)
                <div
                    class="bg-white rounded-2xl border border-gray-200/80 shadow-xs hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col group">
                    <!-- Card Cover Image -->
                    <div class="relative h-48 overflow-hidden bg-gray-100">
                        ...
                        <!-- Price Badge -->
                        <span
                            class="absolute top-3 right-3 px-3 py-1 rounded-lg text-xs font-extrabold bg-white/95 backdrop-blur-md text-gray-900 shadow-sm border border-white/20">
                        ...
                        @if ($event->remainingPlaces() > 0)
                            <span
                                class="absolute bottom-3 left-3 px-2.5 py-1 rounded-md text-[10px] font-bold backdrop-blur-md {{ $event->remainingPlaces() > 0 ? 'bg-emerald-500/90 text-white' : 'bg-rose-500/90 text-white' }}">
                                {{ $event->remainingPlaces() }} places left
                            </span>
                        @else
                            <span
                                class="absolute bottom-3 left-3 px-2.5 py-1 rounded-md text-[10px] font-bold backdrop-blur-md bg-rose-500/90 text-white">
                                Event Full
                            </span>
                        @endif
                    ...
                    <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
                        <div class="space-y-2">
                            <h3
                                class="text-sm font-bold text-gray-900 group-hover:text-indigo-600 transition-colors line-clamp-1">
                                {{ $event->title }}</h3>
